...
public class CreditApplicationReadServiceImpl implements ICreditApplicationReadService {
    ...
    @Override
    public CreditApplicationResponse getById(UUID id) {
        String sql = "SELECT ca.ID as ca_id, ca.CUSTOMER_ID as ca_customer_id, ca.CREDIT_TYPE_ID as ca_credit_type_id, " +
                     ...
                     "ct.NAME as ct_name, " +
                     "c.CUSTOMER_TYPE_CODE, i.FIRST_NAME, i.LAST_NAME, co.COMPANY_NAME " +
                     "FROM CREDIT_APPLICATIONS ca " +
                     "JOIN CREDIT_TYPES ct ON ca.CREDIT_TYPE_ID = ct.ID " +
                     "LEFT JOIN CUSTOMERS c ON c.ID = ca.CUSTOMER_ID " +
                     "LEFT JOIN INDIVIDUAL_CUSTOMERS i ON i.ID = ca.CUSTOMER_ID " +
                     "LEFT JOIN CORPORATE_CUSTOMERS co ON co.ID = ca.CUSTOMER_ID " +
                     "WHERE ca.ID = ? AND ca.DELETED_DATE IS NULL";
        ...
        List<CreditApplicationResponse> items = jdbcTemplate.query(sql, (rs, rowNum) -> {
            ...
            String customerName = "INDIVIDUAL".equals(type) 
